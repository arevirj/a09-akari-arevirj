...
public class ModelImpl implements Model {
  ...
  public boolean searchLamp(int r, int c) {
    ...
    if (c < currentPuzzle.getWidth() - 1) {
      int rowIndex = c + 1;
      while (rowIndex <= currentPuzzle.getWidth() - 1
          && currentPuzzle.getCellType(r, rowIndex) == CellType.CORRIDOR) {
        if (isLamp(r, rowIndex)) {
          return true;
        }
        rowIndex++;
    ...
    if (r < currentPuzzle.getHeight() - 1) {
      int colIndex = r + 1;
      while (colIndex <= currentPuzzle.getHeight() - 1
          && currentPuzzle.getCellType(colIndex, c) == CellType.CORRIDOR) {
        if (isLamp(colIndex, c)) {
          return true;
        }
        colIndex++;
